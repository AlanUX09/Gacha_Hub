...
export default function Header({ onToggleSidebar }) {
  ...
  const getPageTitle = (path) => {
    ...
    if (path.startsWith('/juegos')) return 'Catálogo de Juegos';
    if (path.startsWith('/novedades')) return 'Novedades';
  ...
  return (
    <header className="sticky top-0 z-40 w-full flex items-center justify-between py-3 px-4 md:px-6 border-b bg-[#FAFAFA]/80 backdrop-blur-md border-zinc-200 dark:bg-[#27272A]/80 dark:border-zinc-800 transition-colors duration-300 shadow-sm dark:shadow-black/20">
      ...
      <div className="flex items-center gap-3">
        {/* Toggle Icon (Only visible on md+ where Sidebar exists) */}
        <button
          onClick={onToggleSidebar}
          className="hidden md:flex items-center justify-center p-2 -ml-2 rounded-lg text-zinc-500 hover:text-zinc-900 hover:bg-zinc-200/50 dark:text-zinc-400 dark:hover:text-white dark:hover:bg-zinc-700/50 transition-colors"
          title="Alternar Menú"
        >
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="hidden md:flex items-center justify-center w-10 h-10 rounded-lg border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50 dark:border-zinc-700 dark:bg-[#1F1F22] dark:text-zinc-300 dark:hover:bg-zinc-800 shadow-sm transition-colors"
